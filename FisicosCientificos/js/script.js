...
window.addEventListener('load', ()=>{
  ...
  const displayOptions = () => {
    optionsContainer.innerHTML += `<h3>Por favor presiona el titulo de la categoría</h3>`;
          ...
          charArray.forEach((char, index) => {
            ...
            if (char === button.innerText) {
              ...
              if (winCount == charArray.length) {
                ...
                usuarios[usuarios.length-1]['score'] += 1
                localStorage.setItem('usuarios', JSON.stringify(usuarios))
                
                //block all buttons
                countScore++;
                localStorage.setItem('countScore', JSON.stringify(countScore));
                //Sacamos el ultimo objeto
                let ultimo = usuarios[usuarios.length - 1];
  
                // Nombre del usuario
                let nombreUser=ultimo.user;
                console.log(nombreUser);
                localStorage.setItem('nombreUser', JSON.stringify(nombreUser));
                //Puntajes del usuario
                //let puntosUser=ultimo.score;
